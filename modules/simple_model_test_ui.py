...
class SimpleModelTestUI(QWidget):
    ...
    def create_control_panel(self):
        ...
        # 添加自定义路径按钮
        self.custom_path_button = QPushButton("自定义路径")
        self.custom_path_button.setStyleSheet("""
            QPushButton {
        ...
        """)
        self.custom_path_button.clicked.connect(self.select_custom_dataset_path)
        dataset_layout.addWidget(self.custom_path_button)
        ...
        config_layout.addWidget(self.custom_path_label)
    ...
    def display_results(self, results):
        ...
        # 显示性能摘要
        summary_text = f"性能摘要:\n"
        summary_text += f"• 处理成功率: {results.get('success_count', 0) / max(results.get('total_images', 1), 1) * 100:.1f}%\n"
        summary_text += f"• 平均处理速度: {results.get('fps', 0):.1f} FPS\n"
        summary_text += f"• 推理效率: {1000 / max(results.get('avg_inference_time', 1), 1):.1f} 图像/秒\n"
        
        if results.get('error_count', 0) > 0:
            summary_text += f"• 错误率: {results.get('error_count', 0) / max(results.get('total_images', 1), 1) * 100:.1f}%\n"
        
        self.summary_text.setText(summary_text)
    
    def select_custom_dataset_path(self):
        """选择自定义数据集路径"""
        # 打开文件夹选择对话框
        ...
        if folder_path:
            ...
            if image_files:
                # 添加自定义路径到数据集选项
                custom_name = f"自定义数据集 ({len(image_files)} 张图像)"
                self.dataset_combo.addItem(custom_name, folder_path)
                self.dataset_combo.setCurrentText(custom_name)
                
                # 更新路径显示
                self.custom_path_label.setText(f"路径: {folder_path}")
                self.custom_path_label.setStyleSheet("color: green; font-size: 11px;")
                
                # 更新模型信息显示
                self.model_info_text.setText(f"自定义数据集: {os.path.basename(folder_path)}\n"
                                           f"路径: {folder_path}\n"
                                           f"图像数量: {len(image_files)}\n"
                                           f"支持格式: JPG, PNG, BMP")
            else:
                QMessageBox.warning(self, "警告", f"所选文件夹中没有找到图像文件！\n支持的格式: {', '.join(image_extensions)}")
                self.custom_path_label.setText("未选择有效数据集")
                self.custom_path_label.setStyleSheet("color: red; font-size: 11px;")
