...
class DetectionManager(private val context: Context) {
    ...
    fun detectObjects(image: ImageProxy): List<DetectionResult> {
        if (!isDetecting) return emptyList()
        
        return try {
            // 简化检测逻辑，直接使用模拟检测
            Log.d(TAG, "执行简化检测")
            performSimplifiedDetection()
        } catch (e: Exception) {
            Log.e(TAG, "检测失败", e)
            emptyList()
        }
    ...
    private fun performSimplifiedDetection(): List<DetectionResult> {
        // 简化的模拟检测
        ...
        if (System.currentTimeMillis() % 3 == 0L) {
            detections.addAll(mockDetections)
        }
    ...
    fun detectBlindPath(image: ImageProxy): BlindPathResult? {
        return try {
    ...
    private fun performFallbackDetection(image: ImageProxy): List<DetectionResult> {
        // 回退到模拟检测
        val bitmap = imageToBitmap(image)
...
data class DetectionResult(
    ...
    fun getVoiceMessage(): String {
        return when (riskLevel) {
            ...
            "中" -> "请注意，前方${String.format("%.1f", distance)}米处有$label，请向${direction}方向前进"
